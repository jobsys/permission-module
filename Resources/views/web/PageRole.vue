<template>
	...
	>
		<template #functional>
			<NewbieButton type="primary" icon="PlusOutlined" @click="onEdit(false)" v-auth="'api.manager.permission.role.edit'"
				>新增角色
			</NewbieButton>
		</template>
	</NewbieTable>
	...
	<a-modal v-model:visible="state.showAddScopeModal" @ok="onAddScope">
		<a-form>
...
<script setup>
import { NewbieButton, NewbieEdit, NewbieIcon, NewbieModal, NewbieTable, useTableActions } from "@web/components"
				...
				return useTableActions([
					{
						...
						props: {
							icon: "EditOutlined",
							size: "small",
							auth: "api.manager.permission.role.edit",
						},
						action() {
							onEdit(record)
						},
...
const getForm = () => {
	return [
		...
		},
		{
			key: "name",
			title: "角色标识",
			tips: "必需唯一，用于系统区分，推荐使用字母，数字，中划线组合",
			required: true,
		},
		{
			key: "description",
