<template>
	...
	>
		<template #functional>
			<NewbieButton type="primary" :icon="h(PlusOutlined)" @click="onEdit(false)"
						  v-auth="'api.manager.permission.role.edit'"
			>新增角色
			</NewbieButton>
		</template>
	</NewbieTable>
					...
					>
						<template #bodyCell="{ column, record }">
							...
							<template v-if="column.dataIndex === 'scope'">
								<a-select key="scope" v-model:value="record.value" :options="record.options"
	...
	<a-modal v-model:open="state.showAddScopeModal" @ok="onAddScope">
		<a-form>
			...
			<a-form-item label="数据范围">
				<a-select v-model:value="state.scopeAddForm.value" :options="addScopeOptions"
						  style="width: 200px"></a-select>
			</a-form-item>
		</a-form>
	</a-modal>
